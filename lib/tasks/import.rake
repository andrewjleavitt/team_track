...
namespace :import do
  ...
  task :people, [:path] => :environment do |task, args|
    ...
    CSV.foreach(args.path, options).pluck(:person).uniq.each do |person|
      CreatePerson.new(name: person)
    end
  end

  desc "Imports Projects from a CSV"
  task :projects, [:path] => :environment do |task, args|
    options = {headers: true, header_converters: :symbol}
    CSV.foreach(args.path, options).pluck(:track).uniq.each do |track|
        CreateProject.new(name: track)
    end
  end
